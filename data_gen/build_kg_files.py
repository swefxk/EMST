...
def build_entity_relation_dicts(out_dir, events, sensors, geocells, bands, sources):
    entities = []
    for e in events:
    ...
    for src in sources:
        entities.append(f"source:{src['source_id']}")
...
def build_triples(out_dir, events, sensors, split_ids):
    ...
    for event in events:
        ...
        if event.get("source_id", -1) >= 0:
            triples.append((h, "from_source", f"source:{event['source_id']}"))
        if event["is_true"] == 1:
            triples.extend(
                [
...
def main():
    ...
    truth_events, sources = generate_truth_events(config, rng)
    save_truth(args.out, geocells, bands, truth_events, sources)
    ...
    build_entity_relation_dicts(
        kg_dir, observations, sensors, geocells, bands, sources
    )
